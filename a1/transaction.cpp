...
bool txn_t::validate() 
{
	...
	SHA256_Init(&sha256);
	SHA256_Update(&sha256, this->public_key.data(), this->public_key.size());
	SHA256_Update(&sha256, this->source_addr.data(), this->source_addr.size());
	SHA256_Update(&sha256, this->dest_addr.data(), this->dest_addr.size());
	SHA256_Update(&sha256, this->change_addr.data(), this->change_addr.size());
	SHA256_Update(&sha256, &amount, sizeof(amount));
...
void txn_t::print_txn_hash() const {
	for(u_int32_t i = 0; i < SHA256_DIGEST_LENGTH;i ++){
		printf("%d", this->tx_hash[i]);
	}
	printf("\n");
}
